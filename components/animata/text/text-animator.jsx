...
function animatePhase(controller, units, phase, spec) {
  ...
  units.forEach((unit, index) => {
    ...
    registerAnimation(controller, unit.animate([fromK, toK], {
...
async function runSharedSlideOpacityLoop(
  ...
  const wordOpacityDuration = Math.max(90, Math.round((build.wordOpacityDurationMs ?? 170) * runtime.tileSpeed));
  ...
  const enterPhrase = async text => {
    const { title, units } = makeTitle(stage.ownerDocument, text, spec.target ?? "per-word", c.titleClassName);
    ...
    stage.appendChild(title);
    const animations = [
      registerAnimation(c, title.animate([fromFrame, toFrame], {
        duration: titleDuration,
        easing: enter.easing,
        fill: "forwards",
      })),
      ...units.map((u, i) =>
        registerAnimation(c, u.animate([{ opacity: wordOpacityFrom }, { opacity: wordOpacityTo }], {
...
async function runKineticCenterBuildLoop(
  ...
  const firstWordDuration = Math.max(180, Math.round((build.firstWordDurationMs ?? 360) * runtime.tileSpeed));
  ...
  const betweenPhrasesMs = Math.max(120, Math.round((build.betweenPhrasesMs ?? 220) * runtime.tileSpeed));
  ...
  const buildPhrase = async (phraseWords) => {
    line.innerHTML = "";
    words = [];
    positions = [];
    for (const [index, text] of phraseWords.entries()) {
      ...
      const widths = Array.from(line.children, (n) => (n).offsetWidth);
      ...
      if (index === 0) {
        ...
        animations.push(registerAnimation(c, word.animate([
          start,
          {
        ...
        words.forEach((currentWord, i) => {
          ...
          animations.push(registerAnimation(c, currentWord.animate([
        ...
        animations.push(registerAnimation(c, word.animate([
          ...
          {
            ...buildKineticFrame(mix(startX, targetX, 0.72), 0, 0.998, entryBlurPx * 0.38, 0.84),
            offset: 0.6,
          },
  ...
  const exitPhrase = async () => {
    ...
    const animations = words.map((word, i) => {
      const p = positions[i];
      return registerAnimation(c, word.animate([
...
async function runKineticTopBuildLoop(
  ...
  const computePositions = (heights) => {
  ...
  const buildPhrase = async (phraseWords) => {
    ...
    for (const [index, text] of phraseWords.entries()) {
      ...
      word.textContent = text;
      line.appendChild(word);
      const heights = Array.from(line.children, (n) => (n).offsetHeight);
...
export default function TextAnimator({
  ...
  return (
    <div
      ...
      <div
        ref={stageRef}
        className={cn("text-animation-stage absolute inset-0", stageClassName)}>
        {failed && effectiveSamples[0] ? (
          <h3
            className={cn("text-animation-title text-animation-fallback", titleClassName)}>
            {effectiveSamples[0]}
          </h3>
        ) : null}
      </div>
